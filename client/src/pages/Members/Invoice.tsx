...
export default function Invoice({onClose, isOpen, includeRegistration}: {onClose: ()=>void, isOpen: boolean, includeRegistration?: boolean}){
    ...
    const calcTotal = () => {
        ...
        const registration_fee = membership_type?.registration_fee ?? 0;
        const sub_total = membership_price + addons_price + (includeRegistration ? registration_fee:0);
        ...
        <ReactModal shouldCloseOnOverlayClick shouldCloseOnEsc onRequestClose={onClose} className="flex flex-col p-6 w-[550px] h-[90vh] bg-white-100 rounded-lg" bodyOpenClassName="overflow-hidden" overlayClassName="modal-overlay" isOpen = {isOpen}>
            ...
            <table className="w-full">
                ...
                <tbody>
                    ...
                    {includeRegistration && <TableItem description="Registration Fee" amount={"Rs " + (membership_type?.registration_fee ?? "N/A")} />}
